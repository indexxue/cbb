...
#define QMI8658A_CTRL1_SIM_3WIRE (1u << 7)
...
#define QMI8658A_SPI_IO_MAX 64u

static void qmi8658a_cs_set(qmi8658a_t *dev, uint8_t level)
{
...
static qmi8658a_status_t qmi8658a_bus_error(const qmi8658a_t *dev)
{
    if (dev != NULL && dev->bus == QMI8658A_BUS_SPI) {
        return QMI8658A_ERROR_SPI;
    }
    return QMI8658A_ERROR_I2C;
}

static qmi8658a_status_t qmi8658a_i2c_write_reg(qmi8658a_t *dev, uint8_t reg, uint8_t value)
{
...
static qmi8658a_status_t qmi8658a_spi_write_reg(qmi8658a_t *dev, uint8_t reg, uint8_t value)
{
    if (dev == NULL || dev->spi_tx == NULL || dev->set_cs == NULL) {
        return QMI8658A_ERROR_NOT_INIT;
    }

    uint8_t buf[2];
    buf[0] = QMI8658A_SPI_ADDR_WRITE(reg);
    buf[1] = value;

    qmi8658a_cs_set(dev, 0u);
    if (dev->spi_tx(buf, 2u) != 0) {
...
static qmi8658a_status_t qmi8658a_write_reg(qmi8658a_t *dev, uint8_t reg, uint8_t value)
{
    ...
    if (dev->bus == QMI8658A_BUS_SPI) {
        return qmi8658a_spi_write_reg(dev, reg, value);
    }
    return qmi8658a_i2c_write_reg(dev, reg, value);
}

static qmi8658a_status_t qmi8658a_i2c_read_regs(qmi8658a_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
...
static qmi8658a_status_t qmi8658a_spi_read_regs(qmi8658a_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    if (dev == NULL || data == NULL || len == 0u) {
        return QMI8658A_ERROR_PARAM;
    }
    if (dev->spi_tx == NULL || dev->spi_tx_rx == NULL || dev->set_cs == NULL) {
        return QMI8658A_ERROR_PARAM;
    }
    if (len > QMI8658A_SPI_IO_MAX) {
        return QMI8658A_ERROR_PARAM;
    }

    uint8_t addr = QMI8658A_SPI_ADDR_READ(reg);
    uint8_t tx_dummy[QMI8658A_SPI_IO_MAX];

    memset(tx_dummy, 0, len);

    qmi8658a_cs_set(dev, 0u);
    if (dev->spi_tx(&addr, 1u) != 0) {
        qmi8658a_cs_set(dev, 1u);
        return QMI8658A_ERROR_SPI;
    }
    if (dev->spi_tx_rx(tx_dummy, data, len) != 0) {
        qmi8658a_cs_set(dev, 1u);
        return QMI8658A_ERROR_SPI;
    }
    qmi8658a_cs_set(dev, 1u);
...
static qmi8658a_status_t qmi8658a_read_regs(qmi8658a_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    if (dev->bus == QMI8658A_BUS_SPI) {
        return qmi8658a_spi_read_regs(dev, reg, data, len);
    }
    return qmi8658a_i2c_read_regs(dev, reg, data, len);
}

static uint8_t qmi8658a_pack_ctrl1(qmi8658a_t *dev)
{
    uint8_t ctrl1 = QMI8658A_CTRL1_ADDR_AI;
    if (dev != NULL && dev->spi_3wire) {
        ctrl1 = (uint8_t)(ctrl1 | QMI8658A_CTRL1_SIM_3WIRE);
    }
    return ctrl1;
}
...
static qmi8658a_status_t qmi8658a_init_core(qmi8658a_t *dev)
{
    uint8_t id = 0u;
...
{
    if (dev == NULL || cfg == NULL || cfg->spi_tx == NULL || cfg->spi_tx_rx == NULL || cfg->set_cs == NULL) {
        return QMI8658A_ERROR_PARAM;
    }

    dev->bus           = QMI8658A_BUS_SPI;
    dev->write         = NULL;
    dev->read          = NULL;
    dev->write_read    = NULL;
...
qmi8658a_status_t qmi8658a_init_spi(qmi8658a_t *dev,
                                    ...
                                    qmi8658a_delay_ms_t delay_ms)
{
    qmi8658a_spi_config_t cfg = {0};
    cfg.spi_tx       = spi_tx;
    cfg.spi_tx_rx    = spi_tx_rx;
    cfg.set_cs       = set_cs;
    cfg.delay_ms     = delay_ms;
    cfg.accel_range  = QMI8658A_ACCEL_RANGE_2G;
    cfg.gyro_range   = QMI8658A_GYRO_RANGE_2048DPS;
    cfg.spi_3wire    = false;
    return qmi8658a_init_spi_with_config(dev, &cfg);
}
